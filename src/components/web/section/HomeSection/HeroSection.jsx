...
const AiMessageBubble = ({ message, position }) => {
  ...
  const getEntryAnimation = () => {
    switch (position) {
      case 0: return { y: -20, opacity: 0 };
      case 1: return { x: 20, opacity: 0 };
      case 2: return { y: 20, opacity: 0 };
      case 3: return { x: -20, opacity: 0 };
  ...
  const getConnectorStyle = () => {
    switch (posStyle.arrow) {
      case 'bottom':
        return {
          className: "absolute top-full left-1/2 bg-gradient-to-b from-white to-transparent",
  ...
  return (
    ...
    >
      <div className={`px-5 py-4 border rounded-lg shadow-lg font-medium text-sm md:text-base relative min-w-[200px] max-w-[450px] ${bubbleStyle} text-white`}>
        {message}

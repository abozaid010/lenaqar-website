...
const Header = ({ clientName, clientID, clientEmail }) => {
  ...
  const confirmLogout = async () => {
    try {
      ...
      toast.success(translate("header.logoutSuccess", t?.header?.logoutSuccess));
    } catch (error) {
    ...
    <header className="bg-white shadow-sm p-4 flex justify-between lg:justify-end items-center no-print">
      ...
      <div className="flex items-center space-x-2 sm:space-x-4 ">
        ...
        >
          ...
          <span>{translate("header.sendMessage", t?.header?.sendMessage)}</span>
        </button>
            ...
            >
              {visibleClientId ? (
                ...
                >
                  ...
                  {translate("header.userMenu.settings", t?.header?.userMenu?.settings)}
                </Link>
              ) : (
                <button
              ...
              >
                ...
                {translate("header.userMenu.logout", t?.header?.userMenu?.logout)}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full">
            <h3 className="text-lg font-medium text-gray-900 mb-4">
              {translate("header.logoutConfirm.title", t?.header?.logoutConfirm?.title)}
            </h3>
            <p className="text-gray-600 mb-6">
              {translate("header.logoutConfirm.message", t?.header?.logoutConfirm?.message)}
            </p>
            <div className="flex justify-end space-x-3">
              ...
              >
                {translate("header.logoutConfirm.cancel", t?.header?.logoutConfirm?.cancel)}
              </button>
              <button
                onClick={confirmLogout}
                className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition-colors"
              >
                {translate("header.logoutConfirm.confirm", t?.header?.logoutConfirm?.confirm)}
              </button>
            </div>
          </div>
        </div>

"use client";

import { Copy } from "lucide-react";
import toast from "react-hot-toast";
import { useI18n } from "@/hooks/useI18n";
import {
  normalizeLinkedAutomatedWhatsappList,
  resolveSenderPhoneNumber,
} from "@/lib/whatsapp-messaging-provider";
import { copyToClipboard } from "@/utils/phone-utils";

function phoneDigits(raw) {
  if (!raw) return "";
  return String(raw).replace(/\D/g, "");
}

/** Build unique copyable wa.me links from linked WA accounts (fallback: profile phone). */
export function getSidebarWhatsappLinks(profileData) {
  if (!profileData || typeof profileData !== "object") return [];

  const accounts = normalizeLinkedAutomatedWhatsappList(
    profileData.linked_automated_whatsapp
  );
  const seen = new Set();
  const links = [];

  for (const account of accounts) {
    const digits = phoneDigits(
      resolveSenderPhoneNumber(account) || account.whatsapp_number
    );
    if (!digits || seen.has(digits)) continue;
    seen.add(digits);
    links.push({
      digits,
      display: `wa.me/+${digits}`,
      copyValue: `https://wa.me/${digits}`,
    });
  }

  if (links.length === 0) {
    const digits = phoneDigits(profileData.phone_number);
    if (digits) {
      links.push({
        digits,
        display: `wa.me/+${digits}`,
        copyValue: `https://wa.me/${digits}`,
      });
    }
  }

  return links;
}

const WhatsAppIcon = ({ className }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor" aria-hidden>
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.188z" />
  </svg>
);

export default function SidebarWhatsappLinks({ profileData }) {
  const { translate } = useI18n();
  const links = getSidebarWhatsappLinks(profileData);

  if (links.length === 0) return null;

  const handleCopy = (copyValue) => {
    copyToClipboard(
      copyValue,
      () => toast.success(translate("common.copied")),
      () => toast.error(translate("common.operationFailed"))
    );
  };

  return (
    <div className="mt-0.5 w-full flex flex-col gap-0" dir="ltr">
      {links.map(({ digits, display, copyValue }) => (
        <button
          key={digits}
          type="button"
          onClick={() => handleCopy(copyValue)}
          title={translate("common.clickToCopy")}
          aria-label={`${translate("sidebar.copyWhatsappLink")}: ${display}`}
          className="compact-btn w-full h-5 flex items-center gap-0.5 rounded-sm text-start hover:bg-gray-50 transition-colors group overflow-hidden"
        >
          <WhatsAppIcon className="h-2.5 w-2.5 shrink-0 text-[#25D366]" />
          <span className="min-w-0 flex-1 truncate whitespace-nowrap text-[9px] leading-none text-gray-600 font-mono">
            {display}
          </span>
          <Copy
            className="h-2.5 w-2.5 shrink-0 text-gray-400 group-hover:text-gray-600 transition-colors"
            aria-hidden
          />
        </button>
      ))}
    </div>
  );
}

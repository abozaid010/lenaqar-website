        ...
        {urls.length > 1 && (
          <>
            ...
            <div className="absolute bottom-2 right-2 bg-black/50 text-white text-xs px-2 py-0.5 rounded-full">
              {translate('projectBuildingTypeGallery.imageCounter')?.replace('{current}', String(idx + 1)).replace('{total}', String(urls.length)) || `${idx + 1}/${urls.length}`}
            </div>
          </>
        )}
      </div>
...
export default function ProjectBuildingTypeGallery({ project }: Props) {
  ...
  return (
    <div className="bg-white rounded-lg border p-6">
      <h2 className="text-xl font-semibold text-gray-900 mb-4">{translate('projectBuildingTypeGallery.unitTypeGalleries')}</h2>

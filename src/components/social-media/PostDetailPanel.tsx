...
import { useSocialMediaPostDetail } from "@/hooks/social-media/useSocialMediaPostDetail";
...
export function PostDetailPanel({ postId }: { postId: string }) {
  const { translate, localeUtils } = useI18n();
  const { data, isLoading, isError, error, refetch } = useSocialMediaPostDetail(postId);
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            <div className="text-xs font-semibold text-gray-600">
              {translate("socialMedia.posts.commentsCount")}
            </div>
            <div className="mt-1 text-sm font-semibold text-gray-900">
              {localeUtils.formatNumber(data.comments_count ?? 0)}
            </div>
          </div>
        </div>
      </div>

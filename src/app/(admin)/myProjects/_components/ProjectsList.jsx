  ...
  return (
    <div className="flex items-center gap-1 flex-wrap">
      ...
      {displayPlans.map((plan, index) => (
        <span
          key={index}
          className={`px-2 py-1 text-xs font-medium rounded-full flex items-center gap-1 ${plan.is_default
    ...
    <>
      ...
      <div className="bg-gray-50 flex flex-col gap-4 p-3 relative flex-1 min-h-0 h-full">
                ...
                {translations.cities.length > 0 && (
                  <div className="relative" ref={cityFilterRef}>
                    ...
                    >
                      ...
                      <ChevronDown
                        size={16}
                        className={`text-primary transition-transform ${isCityFilterOpen ? "rotate-180" : ""
                          }`}
                      />
                    </button>
                    {isCityFilterOpen && (
        ...
        <div className="flex flex-col xl:flex-row gap-4 relative flex-1 min-h-0">
              ...
              {isLoading ? (
                ...
                <div className="space-y-3 p-4">
                  {displayList.map((project) => (
                    <div
                      key={project.id}
                      className={`rounded-lg p-3 border-2 transition-all duration-200 relative ${project._pending
                    ...
                    >
                      {project._pending ? (
                        ...
                        <>
                          ...
                          <h3
                            className={`font-semibold text-lg ${selectedProject?.id === project.id
                                ? "text-primary"
                                : "text-gray-800"
                              }`}
                          >
                          ...
                          <div
                            className={`flex items-center space-x-4 text-sm mb-2 ${selectedProject?.id === project.id
                                ? "text-primary"
                                : "text-gray-600"
                              }`}
                          >
          ...
          {displayList.length > 0 && (
            <div
              className={`flex-1 h-fit overflow-hidden bg-white rounded-lg shadow-sm border-2 transition-all duration-300 ${selectedProject
                            ...
                            <div className="space-y-2">
                              {selectedProject.payment_plans.map(
                                (plan, index) => (
                                  <div
                                    key={index}
                                    className={`px-3 py-2 text-sm font-medium rounded-lg flex items-center justify-between ${plan.is_default
                        ...
                        <>
                          ...
                          <div className="flex overflow-x-auto gap-4 px-4 pb-2 mt-4 flex-shrink-0">
                            {selectedProject.phases.map((phase, idx) => (
                              <div
                                key={idx}
                                className={`relative min-w-[120px] max-w-[160px] h-20 rounded-lg overflow-hidden border bg-gray-100 flex-shrink-0 cursor-pointer transition-all duration-200 ${selectedPhaseIdx === idx
                                    ? "ring-2 ring-primary"
                                    : ""
                                  }`}
